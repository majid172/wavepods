...
class AdminController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        // Episode Listening Report
        $episodeReport = Episode::selectRaw("COUNT(listen_count) as count, DATE(created_at) as date_name")->orderBy('created_at', 'desc')
                        ->groupByRaw("DATE(created_at)")->limit(10)
        ...
        // Subscription Report Graph
        $subscriptionReport = Subscription::selectRaw("COUNT(*) as created_at_count, DATE(created_at) as date_name")->orderBy('created_at', 'desc')
                    ->groupByRaw("DATE(created_at)")->limit(10)
                    ->pluck('created_at_count', 'date_name');
        $subscription['labels'] = $subscriptionReport->keys();
        $subscription['values'] = $subscriptionReport->values();

        $newTickets = SupportTicket::with('user')->orderBy('created_at', 'desc')->whereStatus(0)->limit(5)->get();
        return view('admin.dashboard', compact('pageTitle', 'widget', 'withdrawalsChart', 'depositsChart', 'deposit', 'withdrawals', 'userLogins', 'newTickets','episodeReport', 'episode', 'subscriptionReport', 'subscription'));
    }
